use super::{generate_arpeggio, ArpPattern, ARP_DURATION_CHOICES};
...
fn every_step_gets_exactly_one_note_in_order() {
    for pattern in ArpPattern::ALL {
        let notes = generate_arpeggio(pattern, 4, STEPS, &mut rand::rng());
...
fn voices_stay_inside_the_available_range() {
    for pattern in ArpPattern::ALL {
        for voice_count in 1..=8 {
            let notes = generate_arpeggio(pattern, voice_count, STEPS, &mut rand::rng());
            assert!(
...
fn durations_come_from_the_choices_and_never_overlap_the_same_voice() {
    for pattern in ArpPattern::ALL {
        ...
        for note in &notes {
            ...
            assert!(
                note.duration_steps <= ARP_DURATION_CHOICES.into_iter().max().unwrap(),
                "{}",
                pattern.label()
            );
            assert!(
...
fn a_deterministic_pattern_repeats_its_period() {
    let notes = generate_arpeggio(ArpPattern::Up, 4, STEPS, &mut rand::rng());
...
fn empty_inputs_produce_nothing() {
    assert!(generate_arpeggio(ArpPattern::Up, 0, STEPS, &mut rand::rng()).is_empty());
    assert!(generate_arpeggio(ArpPattern::Up, 4, 0, &mut rand::rng()).is_empty());
    assert!(generate_arpeggio(ArpPattern::Random, 0, STEPS, &mut rand::rng()).is_empty());
}

#[test]
fn a_single_voice_still_produces_one_step_notes() {
    let notes = generate_arpeggio(ArpPattern::Random, 1, STEPS, &mut rand::rng());
