...
impl PrimaryScreen {
    const ALL: [(char, Self, &'static str); 4] = [
...
pub(crate) enum ScreenSwitchMenuAction {
    Continue,
    Closed,
...
#[derive(Default)]
pub(crate) struct ScreenSwitchMenu {
    open: bool,
}
...
impl ScreenSwitchMenu {
    ...
    pub(crate) fn handle_key(&mut self, key: KeyEvent) -> ScreenSwitchMenuAction {
        match key.code {
            KeyCode::Esc => {
                self.open = false;
...
pub(crate) fn is_screen_switch_trigger(key: KeyEvent) -> bool {
    key.modifiers == KeyModifiers::CONTROL && key.code == KeyCode::Char('g')
}

pub(crate) fn draw_screen_switch_menu(frame: &mut Frame<'_>, current: PrimaryScreen) {
    ...
    let lines = PrimaryScreen::ALL
        ...
        .collect::<Vec<_>>();
    let area = crate::ui_utils::centered_text_block_rect(frame.area(), TITLE, &lines);
