...
pub(in crate::tui) struct NotepadAudioCache {
    /// MML文字列 → レンダリング済みサンプルのキャッシュ
    pub(in crate::tui) cache: Arc<Mutex<HashMap<String, Vec<f32>>>>,
    /// `cache` の退避順序（先頭が最古）。上限超過時に先頭から追い出す。
    pub(in crate::tui) order: Arc<Mutex<VecDeque<String>>>,
    ...
    pub(in crate::tui) known_disk_hashes: Arc<Mutex<HashSet<u64>>>,
}
