...
impl<'a> TuiApp<'a> {
    fn sort_patch_select_pairs(&mut self) {
        crate::patches::sort_patch_pairs(
    ...
    fn ensure_patch_select_source_order(&mut self) {
        if self.patch_select.patch_all_source_order.is_empty() {
            self.patch_select.patch_all_source_order = self.patch_select.patch_all.clone();
        }
    }

    fn move_patch_cursor_by(&mut self, delta: isize) {
        if self.patch_select.patch_filtered.is_empty() {
            return;
        }
        let max_cursor = self.patch_select.patch_filtered.len().saturating_sub(1) as isize;
        let next_cursor =
            (self.patch_select.patch_cursor as isize + delta).clamp(0, max_cursor) as usize;
        if next_cursor != self.patch_select.patch_cursor {
            self.patch_select.patch_cursor = next_cursor;
            self.patch_select
                .patch_list_state
                .select(Some(self.patch_select.patch_cursor));
            Self::sync_overlay_list_offset(
                &mut self.patch_select.patch_list_state,
    ...
    fn move_patch_favorites_cursor_by(&mut self, delta: isize) {
        ...
        let next_cursor = (self.patch_select.patch_favorites_cursor as isize + delta)
            .clamp(0, max_cursor) as usize;
        if next_cursor != self.patch_select.patch_favorites_cursor {
            self.patch_select.patch_favorites_cursor = next_cursor;
            self.patch_select
                .patch_favorites_state
                .select(Some(self.patch_select.patch_favorites_cursor));
            Self::sync_overlay_list_offset(
                &mut self.patch_select.patch_favorites_state,
    ...
    pub(super) fn start_patch_select_with_initial_patch_name(
        ...
        if self.patch_select.patch_select_sort_order == crate::patches::PatchSortOrder::Category {
        ...
        self.patch_select.patch_query_textarea =
            crate::text_input::new_single_line_textarea(&self.patch_select.patch_query);
        self.patch_select.patch_filtered = crate::tui::filter_patches(
            &self.patch_select.patch_all,
        ...
        self.patch_select.patch_select_focus = PatchSelectPane::Patches;
        self.patch_select.patch_select_filter_active = false;
        self.patch_select.patch_cursor = initial_patch_name
            .map(|patch_name| {
        ...
        self.patch_select.patch_favorites_cursor = 0;
        self.patch_select.patch_list_state = ListState::default();
        self.patch_select.patch_favorites_state = ListState::default();
        ...
        Self::log_notepad_event(format!(
            ...
            self.patch_select.patch_filtered.len(),
            self.patch_select.patch_favorite_items.len(),
    ...
    pub(super) fn refresh_patch_select_favorites(&mut self) {
        ...
        self.patch_select.patch_favorite_items = self.rebuild_patch_select_favorite_items();
    }

    pub(super) fn toggle_patch_select_sort_order(&mut self) {
        self.ensure_patch_select_source_order();
        let selected_patch = self
            .patch_select
            .patch_filtered
            .get(self.patch_select.patch_cursor)
            .cloned();
        let favorite_items = self.patch_select_favorite_items();
        let selected_favorite = favorite_items
            .get(self.patch_select.patch_favorites_cursor)
            .cloned();
        ...
        self.patch_select.patch_cursor = selected_patch
            ...
            .unwrap_or(0);
        let favorite_items = self.patch_select_favorite_items();
        self.patch_select.patch_favorites_cursor = selected_favorite
    ...
    pub(in crate::tui) fn patch_select_favorite_items(&self) -> Vec<String> {
        crate::tui::filter_items(
            &self.patch_select.patch_favorite_items,
    ...
    pub(in crate::tui) fn sync_patch_select_states(&mut self) {
        if self.patch_select.patch_filtered.is_empty() {
            ...
            self.patch_select.patch_list_state.select(None);
        } else {
            self.patch_select.patch_cursor = self
                .patch_select
                .patch_cursor
                .min(self.patch_select.patch_filtered.len() - 1);
            self.patch_select
                .patch_list_state
                .select(Some(self.patch_select.patch_cursor));
            Self::sync_overlay_list_offset(
        ...
        if favorites_len == 0 {
            self.patch_select.patch_favorites_cursor = 0;
            self.patch_select.patch_favorites_state.select(None);
        } else {
            self.patch_select.patch_favorites_cursor = self
                .patch_select
    ...
    fn patch_select_patch_name_for_selection(
        ...
        match focus {
            PatchSelectPane::Patches => self.patch_select.patch_filtered.get(cursor).cloned(),
    ...
    pub(super) fn patch_select_selected_patch_name(&self) -> Option<String> {
        let cursor = match self.patch_select.patch_select_focus {
            PatchSelectPane::Patches => self.patch_select.patch_cursor,
            PatchSelectPane::Favorites => self.patch_select.patch_favorites_cursor,
        };
        self.patch_select_patch_name_for_selection(self.patch_select.patch_select_focus, cursor)
    }
    ...
    pub(in crate::tui) fn patch_select_preview_mml_for_patch_name(
        ...
        let json = Self::build_patch_json_with_filter_query(
            patch_name,
            Some(&self.patch_select.patch_query),
        );
    ...
    pub(super) fn patch_select_preview_mml(&self) -> Option<String> {
        ...
        self.patch_select_preview_mml_for_selection(self.patch_select.patch_select_focus, cursor)
    }
}
