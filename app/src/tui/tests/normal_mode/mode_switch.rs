...
fn handle_normal_question_mark_enters_help_mode() {
    ...
    let result = app.notepad.handle_normal(KeyCode::Char('?'));

    assert!(matches!(result, NormalAction::Continue));
    assert!(matches!(app.notepad.mode, Mode::Help));
}
...
fn handle_normal_w_launches_daw() {
    ...
    let result = app.notepad.handle_normal(KeyCode::Char('w'));

    assert!(matches!(result, NormalAction::LaunchDaw));
    assert!(matches!(app.notepad.mode, Mode::Normal));
}
...
fn handle_normal_v_launches_keyboard() {
    ...
    let result = app.notepad.handle_normal(KeyCode::Char('v'));
...
fn handle_normal_b_requests_loop_browser_without_running_startup_inline() {
    ...
    let result = app.notepad.handle_normal(KeyCode::Char('b'));
...
fn handle_normal_e_requests_config_edit() {
    ...
    let result = app.notepad.handle_normal(KeyCode::Char('e'));
