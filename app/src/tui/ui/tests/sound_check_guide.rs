...
use crate::tui::NOTEPAD_SOUND_CHECK_GUIDE_MESSAGE;

fn set_notepad_guide(app: &mut TuiApp<'_>, saved_date: Option<&str>) {
    app.notepad_sound_check_guide = SoundCheckGuide::new(saved_date.map(str::to_owned));
    let now = Instant::now();
