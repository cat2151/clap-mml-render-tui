use super::*;
use crate::loop_browser::metadata::metadata_path;
use crate::loop_browser::persisted::PersistedDoc;
use crate::loop_browser::random::{load_from as load_random_decks, random_deck_path};
use crate::loop_browser::track_grid::{
    ...
    pub(super) fn reload(&mut self, cfg: &crate::config::Config) {
        ...
        if track_grid_writable && (loaded_grid.needs_migration || grid_changed) {
            if let Some(path) = track_grid_path.as_ref() {
                if let Err(error) = crate::loop_browser::track_grid::save_to(
                    path,
                    &track_grid,
