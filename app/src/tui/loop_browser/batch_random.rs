use super::*;
use crate::loop_browser::random::LoopRandomScope;
...
impl LoopBrowser {
    pub(super) fn randomize_current_measure(&mut self) -> LoopBrowserAction {
    ...
    fn interval_for_candidate(
        ...
        let grid = crate::loop_browser::track_grid::normalize_previous_markers(&grid).0;
        bpm_interval(
            grid[target.track]
                .iter()
    ...
    fn grid_with_batch_selections(
        ...
        crate::loop_browser::track_grid::normalize_previous_markers(&grid).0
    }
    ...
    fn bpm_red_tracks(&self, grid: &LoopTrackGrid) -> HashSet<usize> {
        let target_bpm = crate::loop_browser::time_stretch::select_target_bpm(
            grid.iter()
                .flatten()
                .filter_map(Option::as_ref)
                .filter_map(|clip| self.source_bpm_for_red(clip)),
                ...
                cells
                    ...
                    .any(|clip| {
                        self.source_bpm_for_red(clip).is_some_and(|source_bpm| {
                            crate::loop_browser::time_stretch::exceeds_time_ratio_limits(
    ...
    fn source_bpm_for_red(&self, clip: &LoopTrackClip) -> Option<f64> {
        self.analysis_for_wav(&clip.wav).and_then(|analysis| {
            (analysis.kind != crate::loop_wav_analysis::LoopWavKind::OneShot)
...
fn bpm_interval(bpms: impl IntoIterator<Item = f64>) -> Option<BpmInterval> {
    ...
    for bpm in bpms {
        interval.minimum = interval
            .minimum
            .max(bpm / crate::loop_browser::time_stretch::MAX_TIME_RATIO);
        interval.maximum = interval
            .maximum
            .min(bpm / crate::loop_browser::time_stretch::MIN_TIME_RATIO);
    }
...
fn common_candidate_bpm(candidates: &[Vec<(LoopWavId, BpmInterval)>]) -> Option<f64> {
    ...
    let mut points = vec![crate::loop_browser::time_stretch::TARGET_BPM];
    ...
    points.sort_by(|left, right| {
        (left - crate::loop_browser::time_stretch::TARGET_BPM)
            .abs()
            .total_cmp(&(right - crate::loop_browser::time_stretch::TARGET_BPM).abs())
