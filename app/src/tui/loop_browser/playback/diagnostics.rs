...
pub(in crate::tui) enum StretchStatus {
    Pending,
    Ready {
...
#[derive(Clone, Debug)]
pub(in crate::tui) struct StretchStatusView {
    pub(in crate::tui) source_bpm: Option<f64>,
    pub(in crate::tui) target_bpm: f64,
    pub(in crate::tui) status: StretchStatus,
}

#[derive(Clone, Debug)]
pub(in crate::tui) struct LoopStretchDiagnostics {
    generation: u64,
...
impl LoopStretchDiagnostics {
    ...
    pub(in crate::tui) fn set_status(
        &mut self,
        generation: u64,
    ...
    pub(in crate::tui) fn status_for(
        &self,
        clip: &LoopPlaybackClip,
        target_bpm: f64,
...
pub(in crate::tui) type SharedLoopStretchDiagnostics = Arc<Mutex<LoopStretchDiagnostics>>;

pub(in crate::tui) fn new_shared() -> SharedLoopStretchDiagnostics {
    Arc::new(Mutex::new(LoopStretchDiagnostics::default()))
}

pub(super) fn profile_label(profile: StretchProfile) -> &'static str {
    match profile {
...
pub(in crate::tui) fn duration_seconds(frames: usize, sample_rate: u32) -> f64 {
    frames as f64 / f64::from(sample_rate)
}

pub(in crate::tui) fn log_event(message: impl AsRef<str>) {
    #[cfg(not(test))]
    crate::logging::append_global_log_line(format!("loop-playback: {}", message.as_ref()));
    #[cfg(test)]
    let _ = message;
}

pub(in crate::tui) fn path_label(path: &Path) -> String {
    path.to_string_lossy().replace('"', "\\\"")
}

pub(super) fn rubberband_revision() -> &'static str {
    GIT_REVISION
}
...
mod tests {
    ...
    fn clip() -> LoopPlaybackClip {
        LoopPlaybackClip {
            ...
            span_measures: 1,
            kind: crate::loop_wav_analysis::LoopWavKind::Loop,
            bpm: Some(100.0),
