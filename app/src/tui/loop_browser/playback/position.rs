...
pub(in crate::tui) type SharedPlaybackPosition = Arc<Mutex<Option<ScheduledMeasure>>>;

#[derive(Clone, Copy, Debug)]
pub(in crate::tui) struct ScheduledMeasure {
    measure: usize,
    started_at: Instant,
    duration: Duration,
...
pub(in crate::tui) struct PlaybackBeat {
    pub(in crate::tui) measure: usize,
    pub(in crate::tui) beat: usize,
    pub(in crate::tui) beats_per_measure: usize,
}

pub(in crate::tui) fn new_shared() -> SharedPlaybackPosition {
    Arc::new(Mutex::new(None))
}

pub(super) fn set(shared: &SharedPlaybackPosition, position: ScheduledMeasure) {
    *shared.lock().unwrap() = Some(position);
}

pub(super) fn clear(shared: &SharedPlaybackPosition) {
    *shared.lock().unwrap() = None;
}

pub(in crate::tui) fn current_beat(
...
#[cfg(test)]
pub(in crate::tui) fn set_beat_for_test(
    shared: &SharedPlaybackPosition,
    measure: usize,
    beat: usize,
    beats_per_measure: usize,
