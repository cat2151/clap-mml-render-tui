use super::*;
use crate::daw::{AbRepeatState, CellCache, DawPlayState};

impl DawApp {
    pub(in crate::daw) fn sync_http_grid_snapshot(&self) {
    ...
    pub(in crate::daw) fn sync_http_status_snapshot(&self) {
        let Some(state) = current_state() else {
            return;
        };
    ...
    pub(in crate::daw) fn apply_pending_http_commands(&mut self) {
    ...
    pub(in crate::daw::http_server) fn apply_http_mml(
        &mut self,
        track: usize,
        measure: usize,
        mml: &str,
    ...
    pub(in crate::daw::http_server) fn apply_http_ab_repeat(
        &mut self,
        start_measure: usize,
        end_measure: usize,
    ) -> Result<(), String> {
