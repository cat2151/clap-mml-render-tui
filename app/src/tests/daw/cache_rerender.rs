use super::*;
use crate::daw::{AbRepeatState, CacheState, CellCache, PlayPosition};
...
fn complete_track_rerender_batch_logs_only_after_last_measure_finishes() {
    ...
    let cache = Arc::new(Mutex::new(vec![vec![CellCache::empty(); 3]; 2]));
    ...
    };
    {
        ...
        cache_guard[1][2].state = CacheState::Pending;
...
fn complete_track_rerender_batch_waits_for_last_active_measure_before_logging_done() {
    ...
    let completion_ctx = TrackRerenderBatchCompletionContext {
        batches: Arc::clone(&batches),
        log_lines: Arc::clone(&log_lines),
        cache: Arc::new(Mutex::new(vec![vec![CellCache::empty(); 3]; 2])),
...
fn complete_track_rerender_batch_skips_stale_pending_job_and_reserves_next_measure() {
    ...
    let (cache_tx, cache_rx) = std::sync::mpsc::channel();
    let cache = Arc::new(Mutex::new(vec![vec![CellCache::empty(); 4]; 2]));
    let completion_ctx = TrackRerenderBatchCompletionContext {
        batches: Arc::clone(&batches),
        log_lines: Arc::clone(&log_lines),
        cache: Arc::clone(&cache),
    ...
    };
    {
        ...
        cache_guard[1][3].state = CacheState::Pending;
...
fn complete_track_rerender_batch_respects_cache_render_worker_limit() {
    ...
    let (cache_tx, cache_rx) = std::sync::mpsc::channel();
    let cache = Arc::new(Mutex::new(vec![vec![CellCache::empty(); 5]; 2]));
    let completion_ctx = TrackRerenderBatchCompletionContext {
        batches: Arc::clone(&batches),
        log_lines: Arc::clone(&log_lines),
        cache: Arc::clone(&cache),
    ...
    };
    {
        let mut cache_guard = cache.lock().unwrap();
        for measure in 3..=4 {
            cache_guard[1][measure].state = CacheState::Pending;
...
fn complete_track_rerender_batch_limits_refill_to_one_while_playing() {
    ...
    let completion_ctx = TrackRerenderBatchCompletionContext {
        ...
        log_lines: Arc::clone(&log_lines),
        cache: Arc::clone(&cache),
        play_position: Arc::new(Mutex::new(Some(PlayPosition {
            measure_index: 0,
...
fn start_track_rerender_batch_logs_only_targeted_measures() {
    ...
    assert!(logs.iter().any(|line| {
        line == "cache: rerender start track1 meas 1, meas 3〜4 (random patch update)"
    }));
    assert!(logs
        .iter()
