...
mod windows {
    ...
    pub(crate) struct FastMidiClient {
        mapping: Mapping,
        event: OwnedHandle,
        pid: u32,
    }
    ...
    impl FastMidiClient {
        ...
        pub(crate) fn set_buffer_multiplier(&mut self, multiplier: u8) -> Result<()> {
...
#[cfg(windows)]
pub(crate) use windows::FastMidiClient;
...
#[cfg(not(windows))]
impl FastMidiClient {
    pub(crate) fn connect(_port: u16) -> anyhow::Result<Self> {
    ...
    pub(crate) fn set_buffer_multiplier(&mut self, _multiplier: u8) -> anyhow::Result<()> {
        anyhow::bail!("shared-memory MIDI is only supported on Windows")
    }
}
