...
//!
//! voicing判定はpatchを切り替えるたびに共有メモリ経由で実プラグインを走らせるため
//! 待ち時間が発生する。同じ patch の判定結果は変わらないので、一度判定したものを
//! ファイルにキャッシュし、以降は判定を省いてpatchの準備だけを行う。
//!
...
pub struct VoicingCache {
    #[serde(default)]
    pub entries: HashMap<String, PatchVoicing>,
}

impl VoicingCache {
    pub fn get(&self, patch: &str) -> Option<PatchVoicing> {
        self.entries
            .get(&cmrt_patches::normalize_patch_lookup_key(patch))
            .copied()
    }

    /// 判定結果を登録する。内容が変化したときだけ `true` を返す。
    /// `Unknown` は判定できなかったことを意味するのでキャッシュしない。
    pub fn insert(&mut self, patch: &str, voicing: PatchVoicing) -> bool {
        if voicing == PatchVoicing::Unknown {
            return false;
        }
        let key = cmrt_patches::normalize_patch_lookup_key(patch);
        if key.is_empty() {
            return false;
        }
    ...
    pub fn from_shared_json(text: &str) -> Result<Self> {
...
pub fn save_voicing_cache(cache: &VoicingCache) -> Result<()> {
...
pub fn load_voicing_cache() -> VoicingCache {
    let Some(path) = super::paths::voicing_cache_path() else {
