...
pub fn rename_patch_phrase_store_key(store: &mut PatchPhraseStore, from: &str, to: &str) -> bool {
    if from == to {
        return false;
    }
...
pub fn normalize_patch_phrase_store_for_available_patches(
    ...
    for patch_name in patch_names {
        let Some(resolved) = cmrt_patches::resolve_display_patch_name(pairs, &patch_name) else {
            continue;
        };
...
pub fn touch_patch_favorite(store: &mut PatchPhraseStore, patch_name: &str) {
...
pub fn sync_patch_favorite_order(store: &mut PatchPhraseStore, patch_order: &[String]) -> bool {
    ...
    let mut extras = store
        ...
        .collect::<Vec<_>>();
    extras.sort_by(|left, right| cmrt_patches::compare_patch_names_natural(left, right));
