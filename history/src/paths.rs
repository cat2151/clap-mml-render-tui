...
pub(crate) fn migrate_legacy_history_file(file_name: &str) -> Option<PathBuf> {
...
pub(crate) fn resolved_history_file_path(file_name: &str) -> Option<PathBuf> {
    let path = history_file_path(file_name)?;
    if path.exists() {
        return Some(path);
    }
...
pub(crate) fn patch_phrase_store_path() -> Option<PathBuf> {
