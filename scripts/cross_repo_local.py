...
def lock_ps_entries() -> dict[str, str | None]:
    """Cargo.lock 内の play-server 由来 package を {name: rev} で返す。
...
def cmd_off(args: argparse.Namespace) -> int:
    ...
    if args.keep_lock:
        ...
        stat = git_out(["diff", "--stat", "--", "Cargo.lock"], REPO_ROOT)
        if stat:
            info("未 commit の Cargo.lock 変更を捨てて HEAD の内容へ戻します:")
            info(f"  {stat.strip()}")
        proc = run(["git", "checkout", "--", "Cargo.lock"], check=False)
        if proc.returncode != 0:
            fail("Cargo.lock を復元できませんでした。git status を確認してください。")
        info("Cargo.lock を HEAD の内容へ戻しました。")
...
def cmd_status(args: argparse.Namespace) -> int:
    if SIBLING_DIR.exists() and not args.no_fetch:
        fetch_sibling()
    return report_status(after_off=False)


def report_status(after_off: bool) -> int:
    problems: list[str] = []
    ...
    if patched:
        problems.append(f"Cargo.lock の {', '.join(patched)} が path 参照のまま（source 行が剥がれている）。")
    if len(revs) > 1:
    ...
    info(f"Cargo.lock の rev  : {short(lock_rev)} ({len(entries)} package)")
    ...
    if head is None:
        ...
        if not pushed:
            ...
            problems.append(
                ...
                f"({short(remote)}) より古い。`cargo update -p cmrt-core -p cmrt-server-config` で追従すること。"
            )
