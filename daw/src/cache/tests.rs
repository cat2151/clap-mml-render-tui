...
/// **このモジュールで唯一 env guard を取らないテスト。**
///
/// [`workspace_cache_dir`]（`daw/src/cache.rs:12`）は root を引数で受け取るだけの
/// 純粋なパス計算で、`CMRT_BASE_DIR` も読まないしファイルにも触らない。
/// ここへ `temp_local_dirs` を足しても直列化が増えるだけで守るものが無い。
/// **ファイルを触る処理を足すときは、同時に `temp_local_dirs` を取ること。**
#[test]
...
fn the_chord_row_never_produces_a_cache_job() {
    let (_temp, _env_guard) = temp_local_dirs("cache_chord_row_no_job");
...
fn a_cell_generated_from_the_chord_row_is_queued_even_though_the_cell_itself_is_empty() {
    let (_temp, _env_guard) = temp_local_dirs("cache_chord_row_generated_job");
    let (mut app, cache_rx) = crate::input::tests::build_test_app();
...
fn editing_the_chord_row_invalidates_only_the_cells_generated_from_it() {
    // `invalidate_dependent_cells` は `remove_file(cache_wav_path(..))` で
    // `track{行}_meas{小節}.wav` を**実際に消す**（`daw/src/cache.rs:203`）。
    // guard 無しで走らせると、同時に走る隣のテストの WAV を消す。
    let (_temp, _env_guard) = temp_local_dirs("cache_chord_row_invalidate");
...
/// 実際に通すための道具立て。
///
/// - `temp_local_dirs` で cache dir を pid + nanos のユニークな temp へ逃がす
///   （実 `%LOCALAPPDATA%` を汚さない・隣のテストと `track{行}_meas{小節}.wav` を取り合わない）
...
mod restore_from_metadata {
    ...
    fn a_tempo_only_change_keeps_yesterdays_wav_out_of_the_restored_cache() {
        let (_temp, _env_guard) =
            crate::input::tests::temp_local_dirs("cache_restore_tempo_change");
        let (mut app, _cache_rx) = crate::input::tests::build_test_app();
    ...
    fn a_wav_whose_length_does_not_match_todays_measure_is_still_restored_as_ready() {
        let (_temp, _env_guard) =
            crate::input::tests::temp_local_dirs("cache_restore_length_mismatch");
        let (mut app, _cache_rx) = crate::input::tests::build_test_app();
        let row = FIRST_PLAYABLE_TRACK;
