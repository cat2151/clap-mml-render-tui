...
pub(super) fn draw_help(
    ...
    let help_lines = match mode {
        ...
        _ => vec![
            ...
            Line::from(
                "  Ctrl+T     : 音色選択（chord 行では表示中の演奏 track の init へ反映）",
            ),
            Line::from("  Ctrl+O     : フレーズ履歴（MML 入力のみ）"),
            Line::from(
                "  chord 行  : C で来た元track、なければ最初の生成trackの音色/voicingで試聴",
            ),
