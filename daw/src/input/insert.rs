...
impl DawApp {
    pub(crate) fn commit_insert_cell(&mut self, track: usize, measure: usize, text: &str) -> bool {
    ...
    pub(crate) fn handle_insert(&mut self, key_event: KeyEvent) {
        if key_event.modifiers.contains(KeyModifiers::CONTROL) {
            match key_event.code {
                KeyCode::Char('c') => {
                    self.textarea.copy();
                    cmrt_tui_core::clipboard::set_text(self.textarea.yank_text().to_string());
                    return;
                }
                KeyCode::Char('x') => {
                    self.textarea.cut();
