...
use super::{
    ...
    AbRepeatState, CacheJob, CacheState, CellCache, DawApp, PlayPosition,
};
use cmrt_tui_core::wav_io::load_wav_samples;
...
fn complete_track_rerender_batch_logs_only_after_last_measure_finishes() {
    ...
    let cache = Arc::new(Mutex::new(vec![vec![super::CellCache::empty(); 3]; 3]));
    ...
    DawApp::complete_track_rerender_batch_measure(&completion_ctx, 2, 2);
...
fn complete_track_rerender_batch_waits_for_last_active_measure_before_logging_done() {
    ...
    let completion_ctx = TrackRerenderBatchCompletionContext {
        batches: Arc::clone(&batches),
        log_lines: Arc::clone(&log_lines),
        cache: Arc::new(Mutex::new(vec![vec![super::CellCache::empty(); 3]; 3])),
    ...
    assert!(batches.lock().unwrap()[2].is_none());
    assert_eq!(
        log_lines.lock().unwrap().back().map(String::as_str),
        Some(completion_log)
    );
...
fn complete_track_rerender_batch_skips_stale_pending_job_and_reserves_next_measure() {
    ...
    let (cache_tx, cache_rx) = std::sync::mpsc::channel();
    let cache = Arc::new(Mutex::new(vec![vec![super::CellCache::empty(); 4]; 3]));
    let completion_ctx = TrackRerenderBatchCompletionContext {
        batches: Arc::clone(&batches),
        log_lines: Arc::clone(&log_lines),
        cache: Arc::clone(&cache),
    ...
    };
    {
        let mut cache_guard = cache.lock().unwrap();
        cache_guard[2][2].state = super::CacheState::Pending;
        cache_guard[2][2].generation = 2;
        cache_guard[2][3].state = super::CacheState::Pending;
        cache_guard[2][3].generation = 1;
    }
    batches.lock().unwrap()[2] = Some(TrackRerenderBatch {
        pending: BTreeMap::from([
            (
