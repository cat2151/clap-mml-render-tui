...
//! ## なぜ WAV をコピーするのか
//!
//! 毎小節**同じ**パスを送り直す計測では、サーバーがパスで
//! キャッシュしている可能性を排除できない。ここでは `(行, 小節)` ごとに
...
/// 実測は release ビルドのサーバーで 8 track / 8.6〜10.6%、debug で 21.9〜28.3%。
...
/// # 実測から残す判断
///
/// - 定常の `prepare_ms` は release 8 track で約 200ms（小節の 8.6%）、debug で約 520ms
///   （21.9%）。`STEADY_BUDGET_RATIO` の 0.5 はその 2 倍以上の余裕。
