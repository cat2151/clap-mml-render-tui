...
#[derive(Clone, Copy)]
pub(in crate::http_server) enum RequestHeaderName {
    Origin,
    IfNoneMatch,
}

pub(in crate::http_server) fn request_header_value(
    headers: &[Header],
...
pub(in crate::http_server) fn request_origin(headers: &[Header]) -> Option<String> {
    request_header_value(headers, RequestHeaderName::Origin)
}

pub(in crate::http_server) fn is_allowed_cors_origin(origin: &str) -> bool {
    ALLOWED_CORS_ORIGINS.contains(&origin)
}

pub(in crate::http_server) fn validate_cors_request(
    request: &Request,
...
pub(in crate::http_server) fn with_cors_headers(
...
pub(in crate::http_server) fn with_preflight_cors_headers(
    response: Response<std::io::Cursor<Vec<u8>>>,
    cors_origin: Option<&str>,
) -> Response<std::io::Cursor<Vec<u8>>> {
...
pub(in crate::http_server) fn with_etag_header(
    response: Response<std::io::Cursor<Vec<u8>>>,
    etag: &str,
) -> Response<std::io::Cursor<Vec<u8>>> {
...
pub(in crate::http_server) fn text_response(
    status: u16,
    body: String,
) -> Response<std::io::Cursor<Vec<u8>>> {
...
pub(in crate::http_server) fn empty_response(status: u16) -> Response<std::io::Cursor<Vec<u8>>> {
    Response::from_data(Vec::new()).with_status_code(StatusCode(status))
}

pub(in crate::http_server) fn json_response<T: serde::Serialize>(
    status: u16,
    body: &T,
) -> Response<std::io::Cursor<Vec<u8>>> {
    let header =
