...
pub(in crate::http_server) fn get_snapshot_mml(
    state: &DawHttpState,
    track: usize,
    measure: usize,
...
pub(in crate::http_server) fn get_snapshot_mmls(
    state: &DawHttpState,
...
pub(in crate::http_server) fn get_status_snapshot(
    state: &DawHttpState,
...
pub(in crate::http_server) fn parse_get_mml_query(
    url: &str,
...
pub(in crate::http_server) fn snapshot_mmls_etag(tracks: &[Vec<String>]) -> String {
    let mut hasher = DefaultHasher::new();
    tracks.hash(&mut hasher);
    format!("\"{:016x}\"", hasher.finish())
}

pub(in crate::http_server) fn if_none_match_matches(header_value: &str, etag: &str) -> bool {
    header_value
        .split(',')
        .map(str::trim)
