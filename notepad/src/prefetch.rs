        ...
        F: FnMut(usize) -> Option<String>,
    {
        let (immediate_indices, idle_indices) = match preferred_delta {
            Some(delta) if delta == 1 || delta == -1 => {
                let immediate_indices =
                    cmrt_tui_core::navigation::predicted_navigation_indices_in_direction(
                        current,
                        item_count,
                        delta,
            ...
            Some(delta) => {
                ...
                let idle_indices = cmrt_tui_core::navigation::predicted_navigation_indices(
            ...
            None => (
                cmrt_tui_core::navigation::predicted_navigation_indices(
                    current, item_count, page_size,
                ),
                Vec::new(),
            ),
