...
fn resolve_disk_fallback_samples_recovers_evicted_line_when_hash_is_known() {
    ...
    let app = NotepadScreen::new_for_test(test_config());
    // 起動時 or flush 時の走査で得られる想定のハッシュ集合を模擬する。
    app.audio
        .known_disk_hashes
        .lock()
        .unwrap()
        .insert(cmrt_history::daw_cache_mml_hash(mml));
...
fn flush_notepad_disk_cache_persists_only_current_buffer_lines() {
    ...
    app.flush_disk_cache();

    let cache_dir = cmrt_core::ensure_notepad_cache_dir().unwrap();
    let buffer_line_path = cache_dir.join(format!(
        "{:016x}.wav",
        cmrt_history::daw_cache_mml_hash(buffer_line)
    ));
    let preview_only_path = cache_dir.join(format!(
        "{:016x}.wav",
        cmrt_history::daw_cache_mml_hash(preview_only)
    ));
    assert!(
    ...
    assert!(known_hashes.contains(&cmrt_history::daw_cache_mml_hash(buffer_line)));
    assert!(!known_hashes.contains(&cmrt_history::daw_cache_mml_hash(preview_only)));
}
