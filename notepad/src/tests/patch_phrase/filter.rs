...
fn handle_patch_phrase_slash_then_enter_keeps_filtered_results_for_j_navigation() {
    ...
    assert!(!app.patch_phrase.filter_active);
    assert_eq!(app.patch_phrase.query, "jk");
    assert_eq!(
...
fn handle_patch_phrase_allows_slash_character_in_filter_query() {
    ...
    assert!(app.patch_phrase.filter_active);
    assert_eq!(app.patch_phrase.query, "/n");
    assert_eq!(
        app.patch_phrase_history_items(),
...
fn handle_patch_phrase_left_in_filter_query_does_not_repreview() {
    ...
    let play_state_before = app.playback.session.play_state().lock().unwrap().clone();

    app.handle_patch_phrase(KeyCode::Left);

    assert!(app.patch_phrase.filter_active);
    assert_eq!(app.patch_phrase.query, "b");
    assert!(*app.playback.session.play_state().lock().unwrap() == play_state_before);
}
...
fn handle_patch_phrase_filter_ctrl_a_uses_tui_textarea_default_binding() {
    ...
    assert!(app.patch_phrase.filter_active);
    assert_eq!(app.patch_phrase.query, "Xpad");
}
...
fn handle_patch_phrase_page_up_at_top_does_not_repreview() {
    ...
    assert_eq!(app.patch_phrase.history_cursor, 0);
    assert!(matches!(
        &*app.playback.session.play_state().lock().unwrap(),
        PlayState::Idle
    ));
