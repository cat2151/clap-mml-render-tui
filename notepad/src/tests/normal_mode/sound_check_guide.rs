...
fn show_notepad_sound_check_overlay(app: &mut NotepadScreen<'_>) {
...
fn plain_j_and_k_complete_notepad_sound_check_guide() {
    for key in ['j', 'k'] {
        ...
        assert_eq!(
            ...
            cmrt_tui_core::sound_check_guide::SoundCheckGuidePresentation::Hidden
        );
    }
}
...
fn modified_j_and_arrow_key_do_not_complete_notepad_sound_check_guide() {
    ...
    assert_eq!(
        app.sound_check_guide.presentation(),
        cmrt_tui_core::sound_check_guide::SoundCheckGuidePresentation::Overlay
    );
}
