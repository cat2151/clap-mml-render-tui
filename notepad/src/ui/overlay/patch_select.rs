    ...
    app.patch_select_preview_mml_for_patch_name(patch_name)
        .is_some_and(|mml| {
            cache.contains_key(&mml)
                || disk_hashes.contains(&cmrt_history::daw_cache_mml_hash(&mml))
        })
}
...
pub(crate) fn draw_patch_select(
    ...
    let mut patch_query_widget = cmrt_tui_core::text_input::build_query_textarea_widget(
    ...
    let mut favorites_query_widget = cmrt_tui_core::text_input::build_query_textarea_widget(
    ...
    if app.patch_select.patch_select_filter_active {
        ...
        f.set_cursor_position(
            cmrt_tui_core::text_input::single_line_textarea_cursor_position(area, widget),
        );
    }
