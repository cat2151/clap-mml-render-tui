...
pub(super) fn load_valid_cached_wav(mml: &str, expected_sample_rate: u32) -> Option<Vec<f32>> {
    ...
    let info = cmrt_tui_core::wav_io::read_wav_cache_info(&path).ok()?;
    if info.spec.sample_rate != expected_sample_rate || info.spec.channels != 2 {
        return None;
    }
    let samples = cmrt_tui_core::wav_io::load_wav_samples(&path).ok()?;
...
pub(super) fn scan_valid_cache_hashes(sample_rate: u32) -> HashSet<u64> {
    ...
    for entry in entries.flatten() {
        ...
        let Ok(info) = cmrt_tui_core::wav_io::read_wav_cache_info(&path) else {
            continue;
        };
...
mod tests {
    ...
    /// リダイレクトする（`cmrt_history::test_support::set_local_dir_envs` と同じ仕組み）。
    fn isolated_cmrt_base_dir_env(
        name: &str,
    ) -> (PathBuf, cmrt_history::test_support::TestEnvGuard) {
        let base = isolated_test_dir(name);
        let guard = cmrt_history::test_support::set_local_dir_envs(&base);
        (base, guard)
    }
    ...
    fn hash_is_stable_and_distinguishes_embedded_json_prefix() {
        let a = cmrt_history::daw_cache_mml_hash("cde");
        let b = cmrt_history::daw_cache_mml_hash("cde");
        assert_eq!(a, b);

        let c = cmrt_history::daw_cache_mml_hash(r#"{"Surge XT patch": "x"}cde"#);
        assert_ne!(a, c);
    }
    ...
    fn scan_valid_cache_hashes_includes_only_valid_matching_files() {
        ...
        assert!(hashes.contains(&cmrt_history::daw_cache_mml_hash(valid_mml)));
        assert!(!hashes.contains(&cmrt_history::daw_cache_mml_hash(stale_rate_mml)));
    }
