...
impl LoopBrowser {
    ...
    pub fn save_track_grid(&self) -> anyhow::Result<()> {
        match &self.track_grid_path {
            Some(path) => cmrt_loop_browser_domain::track_grid::save_to(
                path,
                &self.track_grid,
    ...
    pub fn adjust_mixer_volume(&mut self, delta_db: i32) -> LoopBrowserAction {
        ...
        if !cmrt_tui_core::mixer::adjust_volume_db(volume_db, delta_db) {
            return LoopBrowserAction::Continue;
        }
        let next = *volume_db;
    ...
    pub fn toggle_current_cell(&mut self, pad: char) -> LoopBrowserAction {
    ...
    fn log_pad_wav_context(&self, pad: char, wav: &LoopWavId) {
        ...
        let declared_bpm = analysis
            ...
            .map(cmrt_loop_browser_domain::time_stretch::format_bpm)
            .unwrap_or_else(|| "none".to_string());
        let effective_bpm = analysis
            .and_then(|value| value.tempo)
            .map(|tempo| cmrt_loop_browser_domain::time_stretch::format_bpm(tempo.bpm))
            .unwrap_or_else(|| "none".to_string());
        let favorite = self
            .metadata
            .value
    ...
    pub fn insert_current_clip(&mut self, wav: LoopWavId) -> Option<usize> {
        if !self.track_grid_writable
            || self
                .clip_at(self.track_cursor, self.measure_cursor)
                .is_some()
    ...
    pub fn replace_current_clip(&mut self, wav: LoopWavId) -> Option<usize> {
        if !self.track_grid_writable {
            return None;
        }
        let start = self
    ...
    pub fn move_track_cursor_right(&mut self, count: usize) {
        if count == 0 {
            return;
        }
    ...
    pub fn move_track_cursor_down(&mut self, count: usize) {
        if count == 0 {
            return;
        }
