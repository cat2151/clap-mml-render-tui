...
use crate::playback::diagnostics::LoopStretchDiagnostics;
use crate::{LoopBrowser, LoopPlaybackClip};
use cmrt_loop_browser_domain::metadata::LoopWavId;
use cmrt_loop_browser_domain::track_grid::LoopTrackClip;
use cmrt_tui_core::status::base_style;
use cmrt_tui_core::theme::{cursor_highlight_style, MONOKAI_CYAN, MONOKAI_GRAY};
    ...
    let rows = rows
        ...
        .map(|row| {
            let analysis = browser
                ...
                .map(cmrt_loop_domain::loop_wav_analysis::format_analysis)
