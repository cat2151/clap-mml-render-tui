use crate::playback::diagnostics::{StretchStatus, StretchStatusView};

pub fn stretch_label(
    ...
    if source_bpm.is_none() {
        return match stretch_status.map(|view| &view.status) {
            ...
            Some(StretchStatus::Ready { info, .. }) => {
                let seconds = crate::playback::diagnostics::duration_seconds(
                    info.output_frames,
                    info.sample_rate,
                );
    ...
    let source = cmrt_loop_browser_domain::time_stretch::format_bpm(source_bpm);
    let target = cmrt_loop_browser_domain::time_stretch::format_bpm(target_bpm);
    ...
    let source = cmrt_loop_browser_domain::time_stretch::format_bpm(source_bpm);
    let target = cmrt_loop_browser_domain::time_stretch::format_bpm(view.target_bpm);
    match view.status {
        StretchStatus::Pending => format!("… {source}→{target}"),
        StretchStatus::Ready { info, .. } => {
            let prepared_seconds = crate::playback::diagnostics::duration_seconds(
                info.output_frames,
                info.sample_rate,
            );
        ...
        StretchStatus::Error(_) => {
            ...
            if !(cmrt_loop_browser_domain::time_stretch::MIN_TIME_RATIO
                ..=cmrt_loop_browser_domain::time_stretch::MAX_TIME_RATIO)
                .contains(&ratio)
            {
