use super::*;
use cmrt_loop_browser_domain::metadata::metadata_path;
...
impl LoopBrowser {
    pub fn reload(&mut self, loop_dirs: &[String], loop_categories: &[String]) {
        ...
        let random_decks = PersistedDoc::load(
            random_deck_path,
            load_random_decks,
            LoopRandomDeckState::default,
        );
        let mut browser = match cmrt_loop_browser_domain::library::load_index(loop_dirs) {
            Ok(index) => Self::from_index(index, loop_categories, metadata),
            Err(error) => Self {
                error: Some(format!("{error}\ncmrt scan-loops を実行してください")),
                category_keys: category_keys(loop_categories),
                metadata,
                ..Self::default()
            },
        };
        ...
        if track_grid_writable && (loaded_grid.needs_migration || grid_changed) {
            if let Some(path) = track_grid_path.as_ref() {
                if let Err(error) = cmrt_loop_browser_domain::track_grid::save_to(
                    path,
                    &track_grid,
