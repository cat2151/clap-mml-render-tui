use super::*;
use cmrt_loop_browser_domain::random::{save_to as save_random_decks, LoopRandomScope};
use std::collections::HashSet;
use std::path::Component;

impl LoopBrowser {
    pub fn select_random_wav(&mut self) -> LoopBrowserAction {
        ...
        let (effective_bpm, declared_bpm, source) = analysis.map_or_else(
            ...
            |analysis| {
                ...
                (
                    cmrt_loop_browser_domain::time_stretch::format_bpm(tempo.bpm),
                    tempo
                        .declared_bpm
                        .map(cmrt_loop_browser_domain::time_stretch::format_bpm)
                        .unwrap_or_else(|| "none".to_string()),
                    match tempo.source {
                        cmrt_loop_domain::loop_wav_analysis::LoopAnalysisSource::Acid => "acid",
                        cmrt_loop_domain::loop_wav_analysis::LoopAnalysisSource::DurationEstimate => {
                            "duration-estimate"
                        }
                    },
                )
    ...
    pub fn random_candidates(&self, scope: &LoopRandomScope) -> Vec<LoopWavId> {
    ...
    pub fn rebuild_favorite_wav_keys(&mut self) {
    ...
    pub fn save_random_decks(&self) -> anyhow::Result<()> {
        let path = self
            .random_decks
            .path
            .as_ref()
