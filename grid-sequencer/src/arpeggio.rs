...
use crate::{log_line, GridSequencerScreen, PitchDirection, GRID_STEPS};

impl GridSequencerScreen {
    /// step セル上の wheel。音型を1つ送ってアルペジオを引き直す。
    pub(crate) fn cycle_arpeggio(&mut self, instance: usize, direction: PitchDirection) {
        ...
        let notes = generate(pattern, voice_count, GRID_STEPS, &mut rand::rng());
    ...
    ///
    /// カーソルはリストの手前にある扱いで始める。初回の up は先頭（`Up`）、初回の
    /// down は末尾（`Random`）になる。
    fn advance_arp_pattern(&mut self, instance: usize, direction: PitchDirection) -> ArpPattern {
        let next = match (self.arp_patterns.get(&instance).copied(), direction) {
            (Some(current), PitchDirection::Up) => current.next(),
            (Some(current), PitchDirection::Down) => current.previous(),
            (None, PitchDirection::Up) => ArpPattern::default(),
            (None, PitchDirection::Down) => ArpPattern::default().previous(),
        };
