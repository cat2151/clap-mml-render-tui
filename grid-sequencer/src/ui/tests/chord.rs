//! chord mode の描画（和音の行 / ステータス行 / 再起動アナウンス / ヘルプ）。

use super::*;

/// ステータス行は幅で表示を切り替えるので、chord の表示は広い端末で確かめる。
fn render_wide(screen: &GridSequencerScreen) -> String {
...
fn other_rows_keep_their_own_cells_while_the_chord_row_is_playing() {
    ...
    let rendered = render(&screen);
    let second_row = rendered.lines().nth(FIRST_ROW_Y + 1).unwrap();
...
#[test]
fn the_status_line_shows_the_progression_key_and_position() {
    let screen = chord_screen();

    let rendered = render_wide(&screen);

    assert!(
        rendered.contains("chord Key:C# I-IV-V-I [1/4]"),
        "{rendered}"
    );
}

#[test]
fn the_status_line_shows_why_the_chord_mode_could_not_start() {
    ...
    assert!(
        contains_ignoring_spaces(&rendered, "chord: poly patch が見つかりません"),
        "{rendered}"
    );
}

#[test]
fn the_status_line_stays_quiet_while_the_chord_mode_is_off() {
    let screen = GridSequencerScreen::new(None);

    assert!(!render_wide(&screen).contains("chord Key:"));
    assert!(!render(&screen).contains("chord Key:"));
}
