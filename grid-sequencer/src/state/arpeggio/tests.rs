...
/// 和音の instance と 4 voice の instance、Single lane の instance を1つずつ持つ state。
...
fn voice_count_follows_the_chord_voices_of_the_instance() {
    ...
    assert_eq!(state.arp_voice_count(1), 4);
    // Single lane は声部が1つしかないのでアルペジオにならない。
    assert_eq!(state.arp_voice_count(2), 1);
    // 和音の行と範囲外は対象外。
...
fn voice_count_is_zero_without_a_chord() {
    let state = GridState::with_instance_count(3);
    assert_eq!(state.arp_voice_count(1), 0);
...
fn a_two_note_chord_only_offers_the_voices_it_can_render() {
    ...
    assert_eq!(state.arp_voice_count(1), 2);
}
...
fn the_previous_pattern_of_every_lane_is_discarded() {
    ...
    for lane in 0..4 {
        state
            .lane_mut(LaneAddress::new(1, lane))
            .unwrap()
            .pattern
            .draw_span(0, GRID_STEPS - 1);
    }
    assert!(state.apply_arpeggio(1, &[note(0, 0, 1)]));
    for lane in 1..4 {
        assert!(
            lane_steps(&state, 1, lane)
                .iter()
                .all(|step| *step == NoteStep::Rest),
            "lane {lane} kept its old pattern"
        );
    }
    assert_eq!(lane_steps(&state, 1, 0)[0], NoteStep::Attack);
    assert_eq!(lane_steps(&state, 1, 0)[1], NoteStep::Rest);
}
...
fn the_chord_summary_and_a_chordless_grid_are_rejected() {
    ...
    let mut chordless = GridState::with_instance_count(2);
    assert!(!chordless.apply_arpeggio(1, &[note(0, 0, 1)]));
}

#[test]
fn an_unchanged_grid_reports_no_edit() {
    let mut state = chorded_state();
    assert!(!state.apply_arpeggio(1, &[]));
    assert!(state.apply_arpeggio(1, &[note(0, 0, 1)]));
    assert!(!state.apply_arpeggio(1, &[note(0, 0, 1)]));
}
...
fn voices_without_a_lane_are_dropped() {
    let mut state = chorded_state();
    assert!(state.apply_arpeggio(1, &[note(0, 0, 1), note(1, 9, 1)]));
    assert_eq!(lane_steps(&state, 1, 0)[0], NoteStep::Attack);
}
...
fn a_generated_arpeggio_lands_on_every_step_of_the_instance() {
    let mut state = chorded_state();
    let voice_count = state.arp_voice_count(1);
    ...
    for step in 0..GRID_STEPS {
        let attacks = (0..voice_count)
            .filter(|lane| lane_steps(&state, 1, *lane)[step] == NoteStep::Attack)
            .count();
...
fn applying_an_arpeggio_refreshes_the_velocity_display_mask() {
    let mut state = chorded_state();
    assert!(state.apply_arpeggio(1, &[note(4, 2, 1)]));
    let lane_index = state
        .stored_lane_index(LaneAddress::new(1, 2))
        .expect("stored lane");
