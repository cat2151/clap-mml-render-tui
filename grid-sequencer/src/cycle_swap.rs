...
//!    先読みする。ロード中も演奏 bank の render はサーバー側で回り続けるが、
//!    [`crate::sender`] の送信スレッドは1件ごとに同期で待つので、まとめて投げると
//!    timeline イベントの送出がそのぶん遅れる。OFF の周は音色が変わらないので、
//!    現在 bank 上で取り込むだけで済む。
...
impl CycleSwap {
    ...
    /// 何もしないと 16 件が数百 ms のうちに全部キューへ積まれ、送信スレッドが
    /// ロードの同期待ちで塞がりっぱなしになる（サーバーの演奏 bank は止まらないが、
    /// timeline イベントを送る口が同じ1本なので供給が遅れる）。
    /// 2つの条件で間隔を空ける:
    /// - 直前の1件がサーバー側で終わっている（同時に走らせない。IPC の
    ///   request/response スロットも一度に1件しか持てない）
    /// - 前回の送信から1ステップぶん空いている（イベント送出の口を空ける）
