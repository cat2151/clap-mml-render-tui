...
音色置き場の既定値を持たないプラグイン（Vaporizer2。置き場がユーザー固有の設定で決まる。
play-server 側 ADR 0014）は、**本体がインストール済みで組み込みプロファイルがあっても、
`[plugins.<名前>]` に `patches_dirs` を書くまで載らない。** 既存ユーザーの一覧を勝手に変えないための倒れ方。
...
（フィクスチャの代わりに実物の音色を掴んで落ちる）。外から渡せる口:
...
**黙って外す倒れ方そのものは変えない。** 変えるのは「外したことが誰にも見えない」ほう。
**「一覧に出てこない」は一覧を見ていて気づけない**（出ていないものは見えない）ので、
通知は一覧の外に置く。
...
| `log/log.txt` | `patch-load: event=skipped plugin=... reason=no-patches-dirs note="..."`（`event=ready` / `event=error` も同じ形） |
...
- 数え方: `catalog_notice_lines(cfg)`。**画面側では数えない。** app から
  `NotepadScreenParts.catalog_notes` のように渡す
