...
- 状態: 採用（Daily の 2 経路に実装済み。Persistent は未対応）
...
DAW で **「前日の音が混ざっている」**。今日の project（`daily_daw/current.json`）に中身のある行は
1 行だけなのに、演奏ログは 7 行を鳴らしていた（`sent=row2/i0,row3/i1,…`）。キャッシュ WAV の長さから
逆算したテンポは **BPM 120 / 113 / 107 の 3 世代**で、`daily_daw/archive/` の過去ページと一致した。
今日の timeline は BPM120 で刻みながら、行 3〜8 には前日以前の素材を鳴らしていた。
...
キャッシュの「有効性」は 7 か所に分散していて（ファイルの存在 / `CellCache::state` /
`rendered_mml_hash` / `rendered_measure_samples` / `generation` / `samples` /
永続化された `cached_measures[].mml_hash`）、**演奏経路が読むのは最も弱い 1 つ**。
`types.rs` の `set_pending()` が明示する「旧世代の samples を playback fallback 用に残す」方針は
「メモリ内・同一セル・再 render 中」のもので、**「ディスク上・別の行・日をまたいで」へ一般化してはいけない。**
...
**罠:** `remove_dir_all` で grep すると production は 0 件なので「誰も消さない＝消すのは新機能」に着地し、
`remove_file` で grep すると「編集時だけ消す＝既存不変条件の回復」に着地する。動詞の選択で正反対の設計判断が出る。
...
### 共通の底（`apply_project_snapshot_state()`）へ置いてはいけない
...
底へ移すと `same_day_entry_restores_project_cursor_and_daily_cache_without_persistent_writes`
＝「同じ日の再起動で前のキャッシュが Ready に戻る」が赤くなる。
...
この直しを止めていた「『昨日の続き』を壊す」は誤りだった（経緯と教訓は [0019](0019-investigation-stage-acceptance.md)）。
...
| # | 内容 | 状態 |
|---|---|---|
| Persistent | `load_persistent()` に同型の穴。保存ファイルに無い行の WAV が残る | 番人 `a_persistent_load_also_keeps_a_cache_wav_for_a_row_missing_from_the_save_file` が「壊れた振る舞い」を緑で固定している。直したらこれが赤くなる |
...
| 書き込み中の WAV | `write_wav` はアトミックでない。書き込み中に読むと**無音**になる（エラーも出ない）。tmp + rename にすれば窓は消える（play server `cache-player/src/tests/partial_wav.rs`） | 未対応。**「前日の音が混ざる」の説明ではない別の不具合** |
| スロットの選び方 | `小節 index % SLOT_COUNT` なので**ループ長 5・9・13 は余裕 0**。定常ループは実害なしだが、**末尾の小節から演奏開始すると別の小節が鳴る** | 未対応。詳細と直し方は [0012](0012-live-clock-drift-is-absorbed-not-eliminated.md) |
...
`daw/src/daily/tests/stale_cache.rs` と `daw/src/grid_import/tests/stale_cache.rs`。
実機と同じ形（数十ファイル・長さ 2 種）を temp に作って 1 本残らず消えることと、
`*.wav` 以外（`notes.txt`）が残ることを見る。

- `a_failed_rollover_keeps_yesterdays_page_and_its_cache_wav` — 掃除を `Err` の腕や共通の底へ動かすと赤
- `a_rejected_import_into_a_persistent_daw_touches_no_cache_wav` — 掃除をワークスペース判定より前へ出すと赤
- `same_day_entry_restores_project_cursor_and_daily_cache_without_persistent_writes` — 共通の底へ置くと赤
- `a_persistent_load_also_keeps_a_cache_wav_for_a_row_missing_from_the_save_file` — Persistent の穴を塞いだら赤
...
- **ディスクを触る関数へ変えたら、その関数を呼ぶテストを全部 grep すること。**
  ...
  既存テストが**実ユーザーのキャッシュを消す**ようになった。受け皿が
